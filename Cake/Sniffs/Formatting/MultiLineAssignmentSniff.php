 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class Cake_Sniffs_Formatting_MultiLineAssignmentSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        $expectedIndent = ($assignmentIndent + 3);
        $foundIndent    = strlen($tokens[$prev]['content']);
        if ($foundIndent !== $expectedIndent) {
            $error = 'Multi-line assignment not indented correctly; expected %s tabs but found %s';
            $data  = array(
                      $expectedIndent,
                      $foundIndent,
                     );
